...
export default function BSTExamPage() {
  ...
  const baseActions = useMemo(() => (
    <>
      <div
        ...
        className="inline-flex items-center rounded-md border border-border bg-card/60 p-0.5"
      ...
      <div className="min-h-screen">
        ...
        <main className="max-w-5xl mx-auto px-4 py-6">
          ...
          {papersLoading ? (
            ...
            <Card className="flex flex-col items-center justify-center py-20 text-center px-6" data-testid="empty-bst-unauthorized">
              <div className="flex items-center justify-center w-14 h-14 rounded-2xl mb-5 bg-amber-500/10 border border-amber-500/30">
            ...
            <Card className="flex flex-col items-center justify-center py-20 text-center px-6" data-testid="empty-bst-error">
              <div className="flex items-center justify-center w-14 h-14 rounded-2xl mb-5 bg-destructive/10 border border-destructive/30">
            ...
            <Card className="flex flex-col items-center justify-center py-20 text-center px-6" data-testid="empty-bst-papers">
              <div className="flex items-center justify-center w-14 h-14 rounded-2xl mb-5 bg-primary/10 border border-primary/30">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {papers.map(p => (
                ...
                >
                  ...
                  <div
                    className="flex items-center justify-center gap-2 w-full py-2 rounded-xl text-xs font-semibold text-primary bg-primary/10 border border-primary/30 group-hover:bg-primary/15 transition-colors"
        ...
        <main className="max-w-2xl mx-auto px-4 py-8">
          <Card className="p-6 space-y-5 border-primary/20">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-bold text-lg text-foreground">{t.rules}</h2>
            </div>
  ...
  if (examState === "active" && paper) {
    ...
    const activeActions = (
      <>
        <div
          className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border tabular-nums font-mono text-sm font-semibold ${
            isCritical ? "text-destructive border-destructive/40 bg-destructive/10" :
            isLowTime ? "text-amber-400 border-amber-400/40 bg-amber-400/10" :
    ...
    return (
      <div className="min-h-screen flex flex-col" onContextMenu={e => e.preventDefault()} data-nosnippet>
        ...
        <div className="sticky top-0 z-50 bg-card/80 border-b border-border">
          ...
          />
          <div className="max-w-5xl mx-auto px-4 pb-2.5 flex flex-wrap items-center justify-between gap-3 border-t border-border/40 pt-2">
            <div className="flex items-center gap-1.5 p-1 rounded-lg bg-muted/30 border border-border">
              {sectionTabs.map(tab => {
          ...
          <div className="max-w-4xl mx-auto px-4 py-6">
            {currentSection === "A" && (
              <div className="space-y-4">
                <h2 className="text-xl font-semibold text-foreground" data-testid="text-section-title">{t.sectionA} (40 {t.marks})</h2>
                {paper.sectionA.map((q) => (
                  <Card key={q.id} className={answers.sectionA[q.id] ? "border-emerald-500/40" : ""}>
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <p className="font-medium text-sm text-foreground" data-testid={`text-mcq-${q.id}`}>
                          {q.questionNumber}. {q.question}
                        </p>
                        <Badge variant="outline" className="flex-shrink-0">{q.marks} {t.marks}</Badge>
                      </div>
                        ...
                        {q.options.map(opt => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-center gap-3 text-left rounded-lg border px-3 py-2 transition-all ${
                                selected
            ...
            {currentSection === "B" && (
              <div className="space-y-6">
                <h2 className="text-xl font-semibold text-foreground" data-testid="text-section-title">{t.sectionB} (80 {t.marks})</h2>
                {paper.sectionB.map(cs => (
                  <Card key={cs.id}>
                    <CardHeader>
                      <CardTitle className="text-base text-foreground">
                        {isAf ? "Vraag" : "Question"} {cs.questionNumber}
                        <Badge className="ml-2">{cs.totalMarks} {t.marks}</Badge>
                      </CardTitle>
                    </CardHeader>
                      ...
                      {cs.subQuestions.map((sq, si) => (
                        <div key={sq.id} className="space-y-2">
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-sm font-medium text-foreground" data-testid={`text-case-q-${sq.id}`}>
                              {cs.questionNumber}.{si + 1} {sq.question}
                            </p>
                            <Badge variant="outline" className="flex-shrink-0">{sq.marks}</Badge>
                          </div>
                          <Textarea
            ...
            {currentSection === "C" && (
              <div className="space-y-4">
                ...
                {paper.sectionC.map(eq => {
                  ...
                  return (
                    <Card key={eq.id} className={isSelected ? "border-primary/50" : ""}>
                      <CardContent className="p-4 space-y-3">
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline">{eq.marks} {t.marks}</Badge>
                          </div>
                        </div>
                        <p className="text-sm text-foreground" data-testid={`text-essay-${eq.id}`}>{eq.question}</p>
                        {isSelected && (
                          <Textarea
  ...
  if (examState === "results" && examResult) {
    const bandColor = BAND_COLORS[examResult.masteryBand] || "text-foreground";
    ...
    const resultsActions = (
      <>
        ...
        <Button size="sm" onClick={() => { setExamState("select"); setExamResult(null); }} data-testid="button-new-exam">
          {t.selectPaper}
        </Button>
        {baseActions}
      </>
        ...
        <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2 text-foreground">
                <Target className="w-5 h-5 text-primary" /> {t.topicBreakdown}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {examResult.topicBreakdown.map(tb => (
                  <div key={tb.topic} className="space-y-1" data-testid={`topic-result-${tb.topic.replace(/\s/g, '-')}`}>
                    ...
                    <Progress value={tb.percentage} className="h-2" />
                  </div>
                ))}
              </div>
            </CardContent>
                ...
                {examResult.results.sectionA.map((r, idx) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-2.5 rounded-lg border text-sm ${
                      r.correct ? "border-emerald-500/30 bg-emerald-500/5" : "border-destructive/30 bg-destructive/5"
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      {r.correct
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-foreground tabular-nums">{r.earned}/{r.marks}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {examResult.integrityLog.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-base flex items-center gap-2 text-foreground">
                  <Shield className="w-5 h-5 text-primary" /> {t.integrity}
                </CardTitle>
              </CardHeader>
              <CardContent>
