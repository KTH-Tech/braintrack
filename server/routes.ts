...
function isValidSaIdNumber(raw: string): boolean {
  ...
  // Date of birth: YYMMDD (century-agnostic; month 01-12, day 01-31).
  const month = parseInt(id.slice(2, 4), 10);
  const day = parseInt(id.slice(4, 6), 10);
  if (month < 1 || month > 12) return false;
  if (day < 1 || day > 31) return false;

  // Luhn check across all 13 digits.
  let sum = 0;
  let alternate = false;
  for (let i = id.length - 1; i >= 0; i--) {
    let n = id.charCodeAt(i) - 48; // '0' = 48
    if (alternate) {
      n *= 2;
      if (n > 9) n -= 9;
    }
...
const onboardingSchema = z.object({
  ...
  // Learner identity fields captured during onboarding.
  firstName: z.string().trim().min(1).max(100).optional(),
  lastName: z.string().trim().min(1).max(100).optional(),
  // South African ID number: exactly 13 digits, valid YYMMDD date in the
  // first 6 positions, and a valid Luhn check digit. SENSITIVE — persisted to
  // users.idNumber and never returned to any client.
  idNumber: z
    .string()
    .trim()
    .optional()
...
// Accepts SA numbers in any supported form — 0XX, 27XX, +27XX — with optional
// spaces, dashes or brackets, strips non-digit characters before normalising to
// compact E.164 (+27XXXXXXXXX) via toE164.
const saPhoneField = z
  .string()
  .transform((v) => {
    // Remove all non-digit characters except a leading '+' so toE164 can do
    // the normalisation correctly even for display-formatted strings like
    // "+27 82 123 4567" that the Settings formatSAPhone() helper produces.
    const stripped = v.replace(/[^\d+]/g, "").replace(/\++/, "+");
    return toE164(stripped);
  })
  .refine((v) => /^\+27[0-9]{9}$/.test(v), {
    message: "Invalid South African phone number. Use 0XX XXX XXXX, +27XX, or 27XX format.",
  });

const phoneOtpRequestSchema = z.object({
  phoneNumber: saPhoneField,
}).strip();
...
export async function registerRoutes(
  ...
  app.post("/api/onboarding", isAuthenticated, async (req: any, res) => {
    try {
      ...
      try {
        ...
        if (data.firstName) userPatch.firstName = data.firstName;
        if (data.lastName) userPatch.lastName = data.lastName;
  ...
  // Legacy subscribe endpoint — removed. All subscription activation now goes
  // through the Netcash checkout flow (/api/subscribe/netcash/init), the
  // activation-code flow (/api/subscribe/activation-code), or the trial flow
  // (/api/subscribe/start-trial). This stub returns 410 Gone for any stale
  // client traffic so callers fail loudly instead of silently.
  app.post("/api/subscribe", isAuthenticated, (_req, res) => {
    res.status(410).json({ error: "gone", message: "This endpoint has been removed. Use the Netcash checkout or activation-code flow to subscribe." });
  });
  ...
  // School dropdown search. Merges two sources:
  //  1. partnerSchools (DB) — carry a real schoolId used for referral linking.
  //  2. The official DBE EMIS national masterlist (server/data/
  //     sa-high-schools.json, generated by scripts/import-sa-schools.ts) —
  //     8,938 OPEN secondary/combined schools across all nine provinces.
  //     These return schoolId=null (onboarding stores the name free-form)
  //     plus natEmis for future exact matching.
  type SaSchool = { id: number; name: string; province: string; town: string };
  let saSchoolsCache: SaSchool[] | null = null;
  async function loadSaSchools(): Promise<SaSchool[]> {
    if (!saSchoolsCache) {
      const mod = await import("./data/sa-high-schools.json");
      saSchoolsCache = (mod.default ?? mod) as SaSchool[];
    }
    return saSchoolsCache;
  }
  ...
  app.get("/api/schools/search", async (req, res) => {
    try {
      ...
      const partnerNames = new Set(partnerRows.map((r) => r.name.toLowerCase()));
      const master = (await loadSaSchools())
        // Match on school name OR town so "soweto" finds Soweto schools even
        // though none are literally named Soweto.
        .filter((s) =>
          (s.name.toLowerCase().includes(needle) || s.town.toLowerCase().includes(needle)) &&
          !partnerNames.has(s.name.toLowerCase()))
        .slice(0, 15 - partnerRows.length)
        .map((s) => ({
      ...
      res.json({ results: [...partnerRows, ...master] });
    } catch (err) {
...
Create comprehensive study notes for the topic provided.`;
  ...
  // Request phone change OTP — sends a real SMS via Twilio and stores a hashed
  // OTP record. Rate-limited per user (3 / 15 min) and per destination (1 / 30 s).
    ...
    try {
      ...
      if (!isTwilioConfigured()) {
        return res.status(503).json({
          ...
          message: "SMS delivery is not yet configured on this environment. Please contact support.",
        });
      }

      // Rate limit 1: per-user, max 3 OTPs in 15 minutes.
      const fifteenMinAgo = new Date(Date.now() - 15 * 60 * 1000);
      const [userCount] = await db
        .select({ cnt: count() })
        .from(phoneOtpCodes)
        .where(and(eq(phoneOtpCodes.userId, userId), gte(phoneOtpCodes.createdAt, fifteenMinAgo)));
      if (Number(userCount?.cnt ?? 0) >= 3) {
        res.set("Retry-After", "900");
        return res.status(429).json({
          error: "too_many_requests",
          message: "Too many verification requests. Please wait 15 minutes before requesting another code.",
          retryAfter: 900,
        });
      }

      // Rate limit 2: per (user, destination), max 1 OTP in 30 seconds.
      // Scoping to user_id avoids cross-user throttling on shared numbers.
      const thirtySecAgo = new Date(Date.now() - 30 * 1000);
      const [destCount] = await db
        .select({ cnt: count() })
        .from(phoneOtpCodes)
        .where(and(eq(phoneOtpCodes.userId, userId), eq(phoneOtpCodes.phoneE164, phoneE164), gte(phoneOtpCodes.createdAt, thirtySecAgo)));
      if (Number(destCount?.cnt ?? 0) >= 1) {
        res.set("Retry-After", "30");
        return res.status(429).json({
          error: "too_many_requests",
          message: "Please wait 30 seconds before requesting another code for this number.",
          retryAfter: 30,
        });
      }

      // Generate and hash the OTP with scrypt (KDF), so that even if the DB
      // is breached, exhausting 6-digit codes requires ~1M KDF evaluations.
      const salt = randomBytes(16).toString("hex");
      // crypto.randomInt is cryptographically secure; left-pad to 6 digits.
      const code = String(randomInt(0, 1_000_000)).padStart(6, "0");
      // scrypt params: N=16384, r=8, p=1 give ~100ms on a modest server.
      ...
      // Build a bilingual SMS body.
      const [userRow] = await db
        .select({ preferredLanguage: users.preferredLanguage })
        .from(users)
        .where(eq(users.id, userId))
        .limit(1);
      const lang = (userRow?.preferredLanguage ?? "en") as "en" | "af";
      const smsBody = lang === "af"
        ? `Jou BrainTrack verifikasiekode is ${code}. Dit verval oor 10 minute. Moenie hierdie kode deel nie.`
        : `Your BrainTrack verification code is ${code}. It expires in 10 minutes. Do not share this code.`;

      // In development, log the OTP to the console for easy testing.
      if (process.env.NODE_ENV !== "production") {
        console.log(`[DEV] Phone OTP for ${phoneE164}: ${code}`);
      }

      const smsResult = await sendSms(phoneE164, smsBody);
      if (!smsResult.ok) {
        console.error("Twilio SMS failed for phone OTP:", smsResult.error, smsResult.message);
        // Delete the unused OTP row so the failed send doesn't consume rate-limit budget.
        await db.delete(phoneOtpCodes).where(eq(phoneOtpCodes.id, (
          await db.select({ id: phoneOtpCodes.id }).from(phoneOtpCodes)
            .where(and(eq(phoneOtpCodes.userId, userId), eq(phoneOtpCodes.phoneE164, phoneE164), isNull(phoneOtpCodes.consumedAt)))
            .orderBy(desc(phoneOtpCodes.createdAt)).limit(1)
        )[0]?.id ?? -1));
        return res.status(502).json({ error: smsResult.error, message: smsResult.message });
      }
  ...
  // Verify phone change OTP — validates the hashed OTP, enforces attempt cap,
  // writes the verified E.164 number to users.phone, and consumes the record.
  app.post("/api/user/phone/verify-otp", isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Look up the most recent un-consumed OTP for this (user, phone) pair.
      const [row] = await db
        .select()
        .from(phoneOtpCodes)
        .where(
      ...
      const genericError = { error: "invalid_or_expired", message: "Invalid or expired code. Please try again." };

      if (!row) return res.status(400).json(genericError);
      // Increment attempts on all verifiable failure paths (expiry, cap) so
      // the counter accurately reflects all failed verification attempts.
      if (row.expiresAt < new Date() || row.attempts >= 5) {
        if (row.attempts < 5) {
          await db.update(phoneOtpCodes).set({ attempts: row.attempts + 1 }).where(eq(phoneOtpCodes.id, row.id));
        }
        return res.status(400).json(genericError);
      }

      // Verify using scrypt — matches the KDF used at generation time.
      const candidateHash = scryptSync(otp, row.codeSalt, 32);
      const storedBuf = Buffer.from(row.codeHash, "hex");
      const match =
        storedBuf.length === candidateHash.length &&
        timingSafeEqual(storedBuf, candidateHash);

      if (!match) {
        // Increment attempt counter; do NOT reveal which check failed.
        await db
      ...
      // Consume the OTP and persist the verified phone number.
      await db
        .update(phoneOtpCodes)
    ...
    try {
      ...
      if (learnerId) {
        ...
        if (linked.length > 0) {
          ...
          // No learner linked to this parent — the generator renders a
          // "Getting Started" report rather than a near-empty page.
      ...
      // Single source of truth: the shared, rebranded generator handles all
      // layout, brand styling and empty-state handling.
